...
faker = Faker()
@allure.epic("Тестирование транзакции")
@allure.feature("Тестирование транзакции между счетами")
class TestAccountTransactionTemplate:
    ...
    @allure.label("QA Auto", "Nikolay MArtoplyas")
    @allure.title("Перевод 200 едениц между счетами")
    def test_accounts_transaction_template(self, db_session):
        with allure.step("Создание тестовых данных в ьазе данных: счета STan and Bob"):
            stan = AccountTransactionTemplate(user=f"Stan_{faker.random_int(100000, 999999)}", balance=1000)
            bob = AccountTransactionTemplate(user=f"Bob_{faker.random_int(100000, 999999)}", balance=500)
        ...
        def transfer_money(session, from_account, to_account, amount):
            ...
            """
            with allure.step("Получаем счет"):
